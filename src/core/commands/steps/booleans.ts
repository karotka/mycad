...
export async function subtractSolids(run: CommandRun): Promise<StepOutcome> {
  ...
  if (!exact) {
    ctx.log('Subtract failed or was cancelled after running too long. If a body is a sliced mesh remnant, rebuild it as a clean solid first.');
    return 'stay';
  }
...
export async function unionSolids({ ctx, active, data, value }: CommandRun): Promise<StepOutcome> {
  ...
  if (!exact) {
    ctx.log('Union failed or was cancelled after running too long. If a body is a sliced mesh remnant, rebuild it as a clean solid first.');
    return 'advance';
  }
