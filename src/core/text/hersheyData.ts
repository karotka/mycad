/**
 * The Hershey simplex roman font ("rowmans"), as data.
 *
 * A single-stroke font: each glyph is the path a pen walks, not an outline to
 * fill. That is the whole point for a plotter — an outline font engraves the
 * *edges* of each letter, so "A" comes out as two thin triangles rather than an
 * A. It is also why the export reported text as skipped rather than drawing it.
 *
 * The .jhf format, one line per character from space (32) to delete (127):
 * columns 0-4 are the glyph number, 5-7 the vertex count, then two characters
 * giving the left and right bearings, then one character pair per vertex. Every
 * coordinate is its character minus 'R'. The pair " R" means lift the pen.
 *
 * ---
 ...
 * commercial or otherwise, providing that the acknowledgements above are
 * distributed with the font data, that the font data is not sold by itself, and
 * that this restriction is passed on to anyone re-distributing it. See
 * HERSHEY-LICENSE in the original distribution for the full text.
 */

/** One line per character, from space (32) through to 127. */
